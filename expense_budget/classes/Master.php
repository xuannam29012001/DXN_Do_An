...
Class Master extends DBConnection {
	...
	function save_category(){
		...
		if($save){
			$resp['status'] = 'success';
			if(empty($id))
				$this->settings->set_flashdata('success',"Danh mục mới đã lưu thành công!");
			else
	...
	function delete_category(){
		...
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success',"Đã xóa danh mục thành công!");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function save_budget(){
		...
		if($save){
			...
			if($update_balance == 1){
				$resp['status'] ='success';
				$this->settings->set_flashdata('success',"Đã lưu thành công ngân sách!");
			}else{
				$resp['status'] = 'failed';
	...
	function delete_budget(){
		...
		if($del){
			...
			if($update_balance == 1){
				$resp['status'] ='success';
				$this->settings->set_flashdata('success',"Đã xóa thành công ngân sách!");
			}else{
				$resp['status'] = 'failed';
				$resp['error'] = $update_balance;
			}
	...
	function save_expense(){
		...
		if($save){
			...
			if($update_balance == 1){
				$resp['status'] ='success';
				$this->settings->set_flashdata('success',"Đã lưu thành công chi phí!");
			}else{
				$resp['status'] = 'failed';
				$resp['error'] = $update_balance;
			}
	...
	function delete_expense(){
		...
		if($del){
			...
			if($update_balance == 1){
				$resp['status'] ='success';
				$this->settings->set_flashdata('success',"Đã xóa chi phí thành công!");
			}else{
				$resp['status'] = 'failed';
				$resp['error'] = $update_balance;
			}
